...
class GameService:
    ...
    def play_game(vk_user_id: int, branch_id: int, code: str = None, employee_id: int = None):
        ...
        with transaction.atomic():
            ...
            # --- СЦЕНАРИЙ 2: КОД ДНЯ (Каждая 3-я попытка: 3, 6, 9...) ---
            if attempt_num % 3 == 0:
                # Проверяем, есть ли у пользователя активный код доставки
                is_delivery_user = GameService._check_delivery_user(locked_client)
                
                if is_delivery_user:
                    # Доставка: пропускаем код дня, сразу даём монеты
                    logger.info(f"Delivery mode active for client {locked_client.id}, skipping daily code")
                    return GameService._give_regular_coin_reward(locked_client, cooldown)
            ...
            # --- СЦЕНАРИЙ 3: ОБЫЧНАЯ НАГРАДА ---
            logger.info(f"Regular reward for client {locked_client.id}, attempt #{attempt_num}")
            return GameService._give_regular_coin_reward(locked_client, cooldown)
    ...
    @staticmethod
    def _give_regular_coin_reward(client: ClientBranch, cooldown: Cooldown):
        amount = 1000
        
        CoinTransaction.objects.create_transfer(
            client_branch=client,
            amount=amount,
            transaction_type=CoinTransaction.Type.INCOME, # Вместо 'ДОХОД'
    ...
    @staticmethod
    def _give_daily_code_reward(client: ClientBranch, cooldown: Cooldown, code: str, branch_id: int, served_by=None):
        """Логика с кодом дня"""
        ...
        # 2. Расчет награды (до 3000)
        last_transactions = CoinTransaction.objects.filter(
            client=client,
            source=CoinTransaction.Source.GAME,      # Используй константы
            type=CoinTransaction.Type.INCOME
        ).order_by('-created_at')[:2]
        
        current_cycle_sum = sum(t.amount for t in last_transactions)
        
        needed = 3000 - current_cycle_sum
        reward = max(needed, 1000)
        ...
        CoinTransaction.objects.create_transfer(
            ...
            source=CoinTransaction.Source.GAME,
            description=f'Бонус за код дня (+{reward})'
        )
